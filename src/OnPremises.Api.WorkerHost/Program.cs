...
public class Program
{
    ...
    public static IHostBuilder CreateHostBuilder(string[] args) =>
        Host.CreateDefaultBuilder(args)
            ...
            .ConfigureServices((hostContext, services) =>
            {
                ...
                services.AddSingleton<IConfigurationSettings>(provider => new AspNetDynamicConfigurationSettings(provider.GetRequiredService<IConfiguration>(), provider.GetService<ITenancyContext>()));

                services.AddSingleton<IHostSettings>(provider => new HostSettings(provider.GetRequiredService<IConfigurationSettings>()));
                ...
                services.AddSingleton<IRecorder>(c => new CrashTraceOnlyRecorder("OnPremises.Api.WorkerHost", c.GetRequiredService<ILoggerFactory>(), c.GetRequiredService<ICrashReporter>()));
                services.AddSingleton<IEnvironmentVariables, EnvironmentVariables>();
                services.AddSingleton<IServiceClientFactory>(c => new InterHostServiceClientFactory(c.GetRequiredService<IHttpClientFactory>(), c.GetRequiredService<JsonSerializerOptions>(), c.GetRequiredService<IHostSettings>()));
                ...
                RegisterQueueListener<AuditMessage>(services, rabbitMqConfigSection, WorkerConstants.Queues.Audits, "DefaultWorkQueuePolicy", logger);
                // RegisterQueueListener<EmailMessage>(services, rabbitMqConfigSection, WorkerConstants.Queues.Emails, "DefaultWorkQueuePolicy", logger);
                // RegisterQueueListener<ProvisioningMessage>(services, rabbitMqConfigSection, WorkerConstants.Queues.Provisionings, "DefaultWorkQueuePolicy", logger);
                // RegisterQueueListener<SmsMessage>(services, rabbitMqConfigSection, WorkerConstants.Queues.Smses, "DefaultWorkQueuePolicy", logger);
                RegisterQueueListener<UsageMessage>(services, rabbitMqConfigSection, WorkerConstants.Queues.Usages, "DefaultWorkQueuePolicy", logger);
                ...
                string domainEventsExchangeName = rabbitMqConfigSection.GetValue<string>($"{domainEventsExchangeConfigKey}:Name");
                string domainEventsExchangeType = rabbitMqConfigSection.GetValue<string>($"{domainEventsExchangeConfigKey}:Type");

                if (string.IsNullOrWhiteSpace(domainEventsExchangeName))
                {
                    logger.LogCritical("Nombre del Exchange de Eventos de Dominio no configurado en '{ConfigKey}:Name'. No se iniciarán listeners de eventos de dominio.", domainEventsExchangeConfigKey);
                }
                else
                {
                    ...
                    //     string subscriptionName = rabbitMqConfigSection.GetValue<string>($"{fullBindingConfigKey}:SubscriptionName");
                    //
                    //     if (string.IsNullOrWhiteSpace(listenerQueueName) || string.IsNullOrWhiteSpace(routingKey) || string.IsNullOrWhiteSpace(subscriptionName))
                    //     {
                    //         logger.LogWarning(
        ...
        where TMessage : class, IQueuedMessage
    {
        ...
        // {
        //     logger.LogWarning("Nombre de cola no encontrado en configuración en '{ConfigPath}'. Listener para {MessageType} no iniciado.", queueConfigPath, typeof(TMessage).Name);
        //     return;
        // }
